...
// Curated match boards. Each entry: [seed, bossSpritesThatSeedSpawns].
// The boss lists were generated by dry-running each seed's 90s wave (megalodon
// appears in every board ~45s in; levia/whale vary), so pickPoolSeed() can
// preload EXACTLY the boss sprites a board will use — no lazy-load pop-in.
// Regenerate the lists if spawn logic changes.
const MATCH_SEEDS = [
  [73737, ['megalodon','levia','whale']],
  [7,      ['megalodon','whale']],
  [1337,   ['megalodon','whale']],
  [3141,   ['megalodon','whale']],
  [6789,   ['megalodon','whale']],
  [24680,  ['megalodon','whale']],
  [5050,   ['megalodon','levia']],
  [8128,   ['megalodon','levia']],
  [42424,  ['megalodon','levia']],
  [100001, ['megalodon','levia']],
  [42,     ['megalodon']],
  [999,    ['megalodon']],
  [12321,  ['megalodon']],
  [31415,  ['megalodon']],
...
function pickSeat(px,py){
  for (const sX of seats){ if (Math.hypot(sX.x-px, sX.y-py) < 60){
    ...
    pickPoolSeed(); for(let i=0;i<5;i++) spawnFish(false); phase='playing'; running=true; brokeT=0; matchStartT=performance.now(); initAudio(); return;
  } }
}
function drawBots(){
...
function startGame(){
  ...
  document.getElementById('overlay-start').classList.add('hidden');
  document.getElementById('overlay-end').classList.add('hidden');
  if (mode==='tournament'){ matchSeed = dailySeed(); setSeed(matchSeed); }
  else pickPoolSeed();        // free + room play draw a curated board and preload its bosses
